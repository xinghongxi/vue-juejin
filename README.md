...
<mark>	<b>搭建开发环境：</b>使用 Vue 开启一个正式的项目：（生成前端模板)</mark>  
	...
	· npm install
	· npm run dev
	(如果项目跑不起来，执行不了，可以使用  npm rebuild)
...
**db.js** 
	...
	var mysql = require('mysql');   //获取数据库中的数据 必须先引入数据库模块工具
	 var con = mysql.createConnection({ //创建链接
      ...
      password:'root',
      port:'8889',
      database:'Vue_gold_user_info'   //数据库名
	})
	module.exports = con
...
*  MySQL：存储数据信息
*  VUE.js：生成前端模板，构建前端页面
*  vue-router：页面跳转
*  node.js & exrepss：生成后台模板
*  ajax：向服务器发送请求
*  jQuery：dom操作前端页面
*  WebPack：监听前端页面
...
|--邢红喜 掘金网/   # 项目总文件夹
	...
	|--掘金前端/	# 前端工程总文件夹（开发版）
		|--build/  			# webpack文件夹,最终发布的代码存放位置
		|--config/ 			# path位置文件夹
		|--node_modules/    # 模块文件夹,npm加载的项目依赖模块
		...
		|--src/	            # 开发的目录文件夹
				...
				|--router/      # vue-router设置文件夹
						   ...
						   |--index.js      # 引入的所有组件,也就是我当前的路由.文件中的import Hello from '@/components/Hello'@代表在当前根目录下 : src
				|--App.vue      # 入口文件
				|--main.js      # 核心文件（vue工具，./App组件，./router , vuex）
